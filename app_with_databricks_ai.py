"""
Hold Busters Dashboard with Databricks AI Chat Agent
"""
...
if 'selected_model' not in st.session_state:
    # Get default from secrets or use fallback
    default_model = st.secrets.get("databricks", {}).get("model_endpoint", "databricks-dbrx-instruct")
...
# Databricks Workspace Client for AI
@st.cache_resource
def get_databricks_client():
    try:
        return WorkspaceClient(
            host=f"https://{st.secrets.databricks.server_hostname}",
            token=st.secrets.databricks.token
        )
    except Exception as e:
        st.error(f"Workspace client failed: {str(e)}")
        return None
...
# Databricks AI agent
def ask_databricks_ai(user_question, schema_name, model_endpoint):
    try:
        client = get_databricks_client()
        if not client:
            return "Error: Could not connect to Databricks workspace"
...
"""
        ...
        # Build conversation messages for Databricks format
        messages = []
        
        # Add system message as first user message (some models don't support system role)
        ...
        # Add current question if not first message
        ...
        if "does not exist" in error_msg or "not found" in error_msg.lower():
            return f"""Error: Model endpoint '{model_endpoint}' not found.

Available options:
1. Create a model serving endpoint in Databricks
2. Use an existing endpoint (update .streamlit/secrets.toml with the correct name)
...
To check available endpoints, go to:
Databricks Console → Machine Learning → Serving → Model Serving

Error details: {error_msg}"""
        return f"Error communicating with Databricks AI: {error_msg}"
...
def main():
    ...
    st.markdown("### Powered by Databricks + Databricks AI")
    st.markdown("---")
    ...
    # Model selection - Your workspace models
    model_options = [
        "databricks-meta-llama-3-3-70b-instruct",  # Fast & Smart (Recommended)
        "databricks-meta-llama-3-1-405b-instruct",  # Most Capable Llama
        "databricks-meta-llama-3-1-8b-instruct",    # Fastest/Cheapest
    ...
    selected_model = st.sidebar.selectbox(
        ...
        model_options,
        index=default_index,
        help="Select the Databricks Foundation Model to use"
    )
    ...
    # Check Databricks AI client
    client = get_databricks_client()
    if not client:
        st.warning("⚠️ Databricks AI client not available")
    
    # Tabs
    tab1, tab2, tab3 = st.tabs(["💬 AI Chat Assistant", "📊 Dashboard", "📋 Data Explorer"])
    
    with tab1:
        st.subheader(f"🤖 Ask Databricks AI About Your Invoices")
        ...
        for idx, msg in enumerate(st.session_state.chat_history):
            if msg["role"] == "user":
                ...
                st.markdown(f'<div class="chat-message assistant-message"><b>Databricks AI:</b> {explanation}</div>', unsafe_allow_html=True)
                    ...
                    with col_submit:
                        if st.button("Submit Feedback", key=f"submit_feedback_{msg_id}"):
                            if feedback_text:
                                ...
                                # Get AI's acknowledgment
                                with st.spinner("🤔 AI is learning from your feedback..."):
                                    response = ask_databricks_ai(feedback_message, schema_name, selected_model)
                                ...
                                st.success("✅ Feedback submitted! AI will improve.")
                                st.rerun()
                            else:
        ...
        if user_input:
            ...
            with st.spinner("🤔 AI is thinking..."):
                response = ask_databricks_ai(user_input, schema_name, selected_model)
